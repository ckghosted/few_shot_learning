...
class VGG(object):
    ...
    def build_model(self):
        image_dims = [self.img_size_h, self.img_size_w, self.c_dim] ### [32,32,3]
    ...
    def build_cnn(self, input_):
        with tf.variable_scope('cnn', regularizer=l2_regularizer(self.l2scale)):
            self.conv1_1 = self.bn_conv1_1(self.conv_layer(input_, "conv1_1"), train=self.bn_train) ## [-1,32,32,64]
            self.relu1_1 = tf.nn.relu(self.conv1_1, name='relu1_1')
            self.conv1_2 = self.bn_conv1_2(self.conv_layer(self.relu1_1, "conv1_2"), train=self.bn_train) ## [-1,32,32,64]
            self.relu1_2 = tf.nn.relu(self.conv1_2, name='relu1_2')
            self.pool1 = self.max_pool(self.relu1_2, 'pool1')  ## [-1,16,16,64]
            ### Layer 2
            self.conv2_1 = self.bn_conv2_1(self.conv_layer(self.pool1, "conv2_1"), train=self.bn_train) ## [-1,16,16,128]
            self.relu2_1 = tf.nn.relu(self.conv2_1, name='relu2_1')
            self.conv2_2 = self.bn_conv2_2(self.conv_layer(self.relu2_1, "conv2_2"), train=self.bn_train) ## [-1,16,16,128]
            self.relu2_2 = tf.nn.relu(self.conv2_2, name='relu2_2')
            self.pool2 = self.max_pool(self.relu2_2, 'pool2') ## [-1,8,8,128]
            ### Layer 3
            self.conv3_1 = self.bn_conv3_1(self.conv_layer(self.pool2, "conv3_1"), train=self.bn_train) ## [-1,8,8,256]
            self.relu3_1 = tf.nn.relu(self.conv3_1, name='relu3_1')
            self.conv3_2 = self.bn_conv3_2(self.conv_layer(self.relu3_1, "conv3_2"), train=self.bn_train) ## [-1,8,8,256]
            self.relu3_2 = tf.nn.relu(self.conv3_2, name='relu3_2')
            self.conv3_3 = self.bn_conv3_3(self.conv_layer(self.relu3_2, "conv3_3"), train=self.bn_train) ## [-1,8,8,256]
            self.relu3_3 = tf.nn.relu(self.conv3_3, name='relu3_3')
            self.pool3 = self.max_pool(self.relu3_3, 'pool3') ## [-1,4,4,256]
            ### Layer 4
            self.conv4_1 = self.bn_conv4_1(self.conv_layer(self.pool3, "conv4_1"), train=self.bn_train) ## [-1,4,4,512]
            self.relu4_1 = tf.nn.relu(self.conv4_1, name='relu4_1')
            self.conv4_2 = self.bn_conv4_2(self.conv_layer(self.relu4_1, "conv4_2"), train=self.bn_train) ## [-1,4,4,512]
            self.relu4_2 = tf.nn.relu(self.conv4_2, name='relu4_2')
            self.conv4_3 = self.bn_conv4_3(self.conv_layer(self.relu4_2, "conv4_3"), train=self.bn_train) ## [-1,4,4,512]
            self.relu4_3 = tf.nn.relu(self.conv4_3, name='relu4_3')
            self.pool4 = self.max_pool(self.relu4_3, 'pool4') ## [-1,2,2,512]
            ### Layer 5
            self.conv5_1 = self.bn_conv5_1(self.conv_layer(self.pool4, "conv5_1"), train=self.bn_train) ## [-1,2,2,512]
            self.relu5_1 = tf.nn.relu(self.conv5_1, name='relu5_1')
            self.conv5_2 = self.bn_conv5_2(self.conv_layer(self.relu5_1, "conv5_2"), train=self.bn_train) ## [-1,2,2,512]
            self.relu5_2 = tf.nn.relu(self.conv5_2, name='relu5_2')
            self.conv5_3 = self.bn_conv5_3(self.conv_layer(self.relu5_2, "conv5_3"), train=self.bn_train) ## [-1,2,2,512]
            self.relu5_3 = tf.nn.relu(self.conv5_3, name='relu5_3')
            self.pool5 = self.max_pool(self.relu5_3, 'pool5') ## [-1,1,1,512]
            ### flatten
            dim = tf.reduce_prod(tf.shape(self.pool5)[1:])
            self.flattened = tf.reshape(self.pool5, [-1, 512])
    ...
    def train(self,
        ...
        data_train = train_dict[b'data'][0:int(data_len*0.8)].reshape((-1, 3, 32, 32)).transpose([0, 2, 3, 1])
        data_valid = train_dict[b'data'][int(data_len*0.8):int(data_len)].reshape((-1, 3, 32, 32)).transpose([0, 2, 3, 1])
...
class VGG_SMALL(VGG):
    def __init__(self,
                 ...
                 l2scale=0.001,
                 vgg16_npy_path='/data/put_data/cclin/ntu/dlcv2018/hw3/vgg16.npy'):
        super(VGG_SMALL, self).__init__(sess,
                                        model_name,
                                        result_path,
                                        img_size_h,
                                        img_size_w,
    ...
    def build_cnn(self, input_):
        with tf.variable_scope('cnn', regularizer=l2_regularizer(self.l2scale)):
            ...
            self.pool4 = self.max_pool(self.relu4_3, 'pool4') ## [-1,2,2,512]
            ### flatten
            dim = tf.reduce_prod(tf.shape(self.pool4)[1:])
            self.flattened = tf.reshape(self.pool4, [-1, 2048])
        return self.flattened
